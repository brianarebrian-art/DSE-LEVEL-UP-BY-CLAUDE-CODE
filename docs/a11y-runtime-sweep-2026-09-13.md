...
> ### ⚠️ 2026-09-16 第二次更正：對比度探針之前計漏咗祖先嘅 `opacity`
>
> 09-13（同 09-15 重量）嗰陣，`__probe()` 只睇元素**自己**嘅 `opacity`。
> 任何擺喺一個 `opacity < 1` 容器入面嘅字，都當成完全不透明咁計 ——
> 量到嘅係宣告值，唔係螢幕上見到嘅顏色。
>
> **點樣發現：** Focus 專注燈（`Shift + F`）將非聚焦區擺喺 `opacity: .25`，
> 開住燈掃，探針照樣報「0 失敗」。一個報唔到已知會變淡嘅情況嘅探針，
> 同 09-15 嗰支量唔到爆版嘅 `__ovf()` 係同一個病。
>
> **已修（`scripts/a11y/runtime-probe.js`）：**
> 由元素沿祖先鏈逐層合成背景同 `opacity`（Porter-Duff），到畫布為止；
> 跳過停用控制項（WCAG 1.4.3 豁免「未啟用介面元件」入面嘅文字）；
> 跳過仍然行緊動畫嘅元素，另加 `await __settle()` 等入場動畫做完先量。
> 自測由兩紅兩綠變三紅三綠，新增嗰一對就係 opacity：
> 黑字喺 `opacity:.25` 容器入面（實際 1.8:1）要紅、`opacity:.9`（17:1）要綠。
> 迴歸鎖：`lib/__tests__/overflow-probe.test.mts` 測試 ④⑤。
>
> **用新探針重量（2026-09-16，375px，`visualViewport.width` 實測 375）：**
>
> | 範圍 | 對比度失敗 | 爆版 |
> |---|---|---|
> | 35 條路由 × 淺色／暗色 = 70 次（預設狀態） | **0** | **0** |
> | 桌面闊度（`vv` 704）11 條路由補掃 | **0** | 0 |
>
> 結論同之前一樣，但今次係量出嚟嘅。
>
> **一個要講清楚嘅例外 —— Focus 專注燈開住嗰陣：**
> 喺 `/practice` 實測，非聚焦區 7 個文字元素跌到 **1.4–1.55:1**（返回／科目名／
> 第 N 題／計時／休息吓等）。呢個係功能本身要做嘅事，唔係 bug，但**唔可以**
> 當成「全站 AA」嘅一部分。三個緩解已實測：預設關（`dse_focus_light`）、
> `Shift + F` 即開即熄、鍵盤聚焦或 hover 時嗰一區即刻返 `opacity: 1`
> （實測 0.25 → 1 → 失焦返 0.25；兩條 CSS 規則都有 `html.focus-light` 前綴，
> 熄燈時對任何人零影響）。
>
> **一個會誤導嘅中途狀態：** 計埋祖先 opacity 之後，載入後太早量會量到入場動畫
> 途中嘅一刻 —— `/practice` 張卡（`ml-q-enter`）1.2 秒時量到 4.28:1，
> 等佢做完（373ms）再量係 4.98:1。所以掃之前一定要 `await __settle()`。
