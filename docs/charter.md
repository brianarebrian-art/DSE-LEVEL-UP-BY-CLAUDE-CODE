> **📍 本文件係 DSE LEVEL UP 憲章嘅唯一正本（canonical）。**
> 2026-08-25 由 `~/Downloads/CLAUDE.md` 移入 repo，納入 git 追蹤 ——
> 此後每次修訂都有 commit、有 diff、有 `git blame`。
>
> `~/Downloads/CLAUDE.md` 已改為指標檔（`@import`），**唔好喺嗰邊改內容** ——
> 兩份會漂移，正正就係 §8.1 記低嗰個「憲章自己同自己打架」。

# DSE LEVEL UP — CLAUDE.md
# 戰場只有一個：香港中學文憑試（DSE）
# 最後更新：2026-09-26 | §12 改為機器閘自動入庫＋事後退回；§1 新增 2.1 不考之地（Yuna 指示）
# 前次：2026-09-26 | 新增 §18 取消簽署制度、§19 Notion 第二大腦（Yuna 指示）
# 前次：2026-09-09 | §7.2 新增：反思鎖剷除，兩個月實驗（2026-11-09 覆檢）
# 前次：2026-09-09 | §16.E 約束 5 Brian 副署 —— 雙簽完成 ✅；一節題數 20→10（§7.1）
# 前次：2026-09-09 | Yuna 補簽 2026-09-05 撤銷收費框架 —— 該次修訂雙簽完成 ✅
# 前次：2026-09-04 | Brian ＋ Yuna 雙簽（§3／§5／§8／§16.E 修訂，新增 §17）
# 前次：2026-08-25 | Brian 簽署停火令（§16 五條永久紅線）
# 前次：2026-08-22 | Yuna 核准（§8.1 gamification 解禁）

---

## 1. 最高憲章（不可違反）

1. **學生成效與長期信任為最高原則。** 任何決策若與憲章衝突，一律以學生成效優先。
2. **戰場只有一個：香港 DSE。** 絕對禁止建議全齡層、亞太區擴張、改名 Aethel / WisdomPath / 化城避風港 / Buff 補給艙等已被否決內容。
   - **2.1 不考之地（2026-09-26，Yuna，按 §18）。** 不屬 DSE 考核範圍的內容，可以放在網站內的「不考之地」（`/off-syllabus`），標語「純粹為樂趣而學」。
     界線：對象仍是香港 DSE 考生，全齡層、亞太區擴張照樣禁止；入口只在三橫選單及桌面側欄最尾，不放首頁或其他當眼位置；
     內容不計分、不記進度；§5 成本死鎖照樣適用；每加入一項內容，由創辦人決定。
     第一項是新來港香港日常廣東話（`/cantonese`）。本條取代 `docs/charter-amendment-2026-09-19-DRAFT.md` 的三個選項。
3. **絕對免費原則（永久有效）。**（2026-09-05 Brian 裁決，撤銷 2026-09-04 之「核心免費原則＋Plus 自願支持計劃」修訂）
   全部功能、全部科目、無限題數，一律免費。**冇付費層、冇每日題數上限、冇任何形式嘅收費、訂閱、廣告或付費牆。**
   - **3.1 冇額度。** 2026-09-04 修訂曾引入「每日 40 題」上限，該上限**從未上線**，現已連同整套收費框架一併撤銷。`entitlements.ts` 維持完全開放。
   - **3.2 已剷除嘅嘢（存檔，防止重建）。** Stripe 接線、`/confirm-payment`、`/thank-you`、`lib/payment/`、每日額度計數、五張付費表（`plus_entitlements`／`purchases`／`consent_logs`／`plus_devices`／`site_config`）全部刪除，見 `supabase/migrations/0015_drop_payment.sql`。**剷之前逐張數過，五張表合共只有 1 行（一個從未被讀取嘅 feature flag），冇一筆真實交易。**
   - **3.3 點解要連憲章一齊改。** 2026-09-04 嗰次修訂係 Brian ＋ Yuna 雙簽寫入憲章嘅。淨係剷代碼、留住條文，下一個 session 讀憲章就會照 §8.2 白名單再起一次 —— 呢個模式憲章自己喺 §8.1 同 §8.2 各記低過一次（「唔喺憲章度調和，每個新 session 都會…重新爭論一輪」）。所以代碼同條文同一日一齊改。

4. **零版權侵犯。** 所有題目必須使用「原創平行改寫法 (Archetype Masking)」，更換數字、情境、人名，精準保留底層考核邏輯。Footer 每頁必須顯示：「© DSE Level Up 2026. Not affiliated with HKEAA. All questions are independently rewritten for educational purposes.」
5. **每月總技術成本死鎖 US$200 以內。**（2026-07-29 Brian 拍板，2026-08-20 Yuna 確認；舊值 HK$180.81 作廢。與 `scripts/budget-guard.mjs` 一致 —— 兩者將來要一齊改。）嚴禁調用任何需付費嘅第三方 API，嚴禁**新增**套件（尤其 Drizzle ORM、Supabase Auth、Chart.js、D3、Recharts）。
   - ⚠️ **Better Auth 唔屬於呢張禁單** —— `better-auth@^1.6.23` 已經安裝並已接線（`lib/auth/better-auth.ts`、`lib/auth/client.ts`、`lib/auth/session.tsx`、`app/api/auth/[...nextauth]/route.ts`、`app/(auth)/sign-in|sign-up`），係一個由 `NEXT_PUBLIC_AUTH_BACKEND` 控制嘅第二 backend，預設仍然係 Auth.js v5。**唔准當佢係幻覺刪走。**
   - ⛔ **原「例外一：Stripe」已於 2026-09-05 撤銷。** `stripe` 套件已 `npm uninstall`，`scripts/budget-guard.mjs` 之 `EXCEPTIONS` 已同步收回 —— 即係話下次有人 `npm i stripe`，個閘會即刻嗌，而唔係靜靜哋放行一個已經廢除嘅例外。

---

## 2. 項目定位與路徑

- **項目名稱：** DSE LEVEL UP（唯一名稱，非 Aethel / 學無止境 / WisdomPath）
- **官方連結：** https://dse-level-up-by-claude-code.vercel.app/
- **目標受眾：** 12-18 歲香港 DSE 考生（P1 邊緣分數考生、P2 自修生、SEN/基層學生、前線老師/SENCO）
- **創辦人模式：** 業餘娛樂模式，每週一次異步同步
- **生產紀律：** 新題過機器閘即自動上線，創辦人事後覆核，有問題即退回（2026-09-26 Yuna 決定，詳見 §12）。
  （原文「機器永不自動入庫」已由該決定取代；舊文保留於 §12.3 存檔。）

---

## 3. 技術棧真實版（2026-07-16 鎖定）

- **Framework:** Next.js 16.2.9 + React 19.2.4
- **Styling:** Tailwind CSS v4
- **Auth:** 雙 backend，由 `NEXT_PUBLIC_AUTH_BACKEND` 揀。**無 Supabase Auth。**
  - 預設：Auth.js v5（Google OAuth，JWT strategy，**唔需要資料庫**）
  - 可切換：Better Auth（Google ＋ **email/password**）—— 2026-09-07 Brian 裁決要開
  - ⚠️ 原條文寫「**無 Better Auth**」，同 §5 嗰段「已經安裝並已接線…唔准當佢係幻覺刪走」
    直接打架。§5 嗰段先啱：`better-auth@^1.6.23` 一直喺 `package.json`，
    `lib/auth/{better-auth,client,session,server}.ts` 全部接咗線，
    cutover runbook 喺 `FLIP-TO-BETTER-AUTH.md`。呢度更正。

#### 開 email/password 之前必須成立（2026-09-07）

Better Auth 一開，`user` 表就會存**學生電郵 ＋ 密碼 hash**。三件事同日處理：

1. **私隱頁唔可以再講「我哋唔會將你嘅電郵地址存入資料庫」。**
   已改為由 `betterAuthEnabled` 衍生 —— 兩個狀態各有一段真文案，
   唔會出現「代碼改咗但私隱政策漏咗改」嗰個窗口。
2. **刪帳號要清埋 Better Auth 嘅表。** 佢哋用 `id`／`userId`／`identifier`，
   **唔係** `user_id`，塞入原本個迴圈會撞 42703 令成個抹除 500。
   已加 `BETTER_AUTH_TABLES` 獨立處理（`verification` 由 email 做 key）。
3. **`getSyncUserId()` 解析唔到 Google `accountId` 嗰陣一律回 `null`。**
   原本會回落 Better Auth 自己個 user id —— 而現有 **169 個帳號**
   （2026-09-07 實測）嘅雲端進度全部 keyed on Google `sub`，換咗 key 就永久失聯，
   而且【冇聲】。回 `null` 只係「今次唔同步」，本機資料一個字都冇少。

迴歸鎖：`lib/__tests__/email-auth-readiness.test.mts`。
- **Middleware:** proxy.ts — **無 middleware.ts**
- **Database:** Supabase PostgreSQL。**兩種存取模式，唔可以混淆：**
  - **寫入 ＋ 一切用戶數據：僅經 server-only `getServiceSupabase()`**（service role key 永不入瀏覽器）
  - **題庫讀取：瀏覽器直連**（2026-09-05 新增，見 §3.1）
- **Data:** 100% localStorage（客戶端），Supabase 只作 server-side 備份

### 3.1 題庫瀏覽器直連 —— 2026-09-05 新增

**決策：** Brian 2026-09-05：「為咗盡量慳住用 Edge Request，將所有題目搬入去
Supabase……user 每一次做題目就呼叫 Supabase 拎題目出嚟」「即刻做瀏覽器直連
Supabase，唔經 Vercel」。

**點解要改憲章而唔係淨係改代碼：** 原條文寫死「僅經 server-only
`getServiceSupabase()`」。瀏覽器直連係一個【新嘅存取模式】，代碼落咗而條文冇跟，
下一個 session 讀憲章就會當佢係違規而拆走 —— §8.1 同 §8.2 各記低過一次呢個模式。

**准許嘅範圍（只此一項）：** `questions` ＋ `question_bank_versions` 兩張表，
**只讀**，經 `NEXT_PUBLIC_SUPABASE_ANON_KEY`。

**約束（全部生效）：**

1. **只限題目內容。** 題目本身已經係公開內容 —— 而家 build 入 JS chunk，
   view-source 就攞得晒，包括 `correctIndex`。搬上雲【冇新增任何曝露】。
   **任何用戶數據（`user_progress`／`user_settings`／`profiles`）一律維持
   server-only**，唔准用同一個 anon key 開多一張表。§16.E 嘅約束不受本節影響。
2. **anon 只有 SELECT。** `revoke insert, update, delete …`（`0017`）＋ RLS 零寫入
   policy，兩層各自獨立成立。題庫只可以由 repo 經 §12 流程寫入，靠嘅就係呢兩層 ——
   學生（或任何攞到 anon key 嘅人）改唔到一隻字。
   ⚠️ Supabase 有 `ALTER DEFAULT PRIVILEGES`，新表自動畀齊 anon 全套 DML，
   所以 `grant select` **唔構成收窄**，一定要顯式 `revoke`。呢點喺 `0016` 原稿
   寫錯過，實測捉返（見 `0017` 檔頭）。
3. **`data/questions/*.ts` 仍然係唯一正本。** 雲端係【衍生鏡像】，
   由人手執行嘅 `scripts/qbank/sync-questions.mts` 單向寫入（repo → 雲）。
   **反方向永不存在，亦唔准加** —— 一旦容許雲 → repo，改一條題目就會變成
   一個冇 diff、冇 blame、冇實名簽署嘅動作，而學生睇到嘅嘢已經變咗。
4. **靜態 chunk 回落路徑唔准拆。** 離線、未設 key、Supabase 死、學生封鎖咗
   IndexedDB —— 四種情況都靠佢。chunk 本來 build 好擺喺 CDN，雲端行得通嗰陣
   一個 byte 都唔會攞，所以留住佢嘅代價係零。
   迴歸鎖：`lib/__tests__/question-cloud.test.mts`。
5. **必須有 IndexedDB 版本比對。** 實測每科 gzip 85–186KB，每次練習都重攞
   即係免費額度 5GB／月 ≈ 28,000 次科目載入就爆 —— 爆咗唔係慢啲，係停止服務，
   同 §5 成本死鎖直接對撞。比對版本號之後，回頭客每次練習題目流量係
   **32 bytes**（實測）。
6. **`@supabase/supabase-js` 唔准入瀏覽器 bundle。** 用 `fetch` 直接打
   PostgREST，bundle 加零。呢點同 §5「嚴禁新增套件」同一個理由。
- **ORM:** 無 Drizzle ORM
- **Charts:** 純 SVG + Tailwind — **禁 Chart.js / D3 / Recharts**
- **Build:** `--webpack`，dev port 3001
- **Testing:** `npm test`（必須報告 pass/fail count）

---

## 4. Integration Requirement（定義完成標準）

**新模組未完成，直到已 wire 入 live app 並可於運行中 UI 手動驗證。**

- 絕對禁止交付僅有 passing unit tests 嘅 standalone modules。
- 同一 session 內必須同時交付：import + call site + UI 觸發點（route / mode selection / control binding）。
- 超過 3 個檔案嘅變更，必須先進入 Phase 1（read-only audit），產出 impact report，等 Brian/Yuna greenlight 後先准進入 Phase 2（執行）。

---

## 5. Language & Comment Rules（術語紅線）

**所有代碼註釋、識別符、提交內容必須使用標準書面語（書面語）或英文——絕對禁止粵語口語（口語）形式。**

- 適用範圍：題庫檔案、topic names、inline comments、UI 文案（解析層）
- **UI 情感層**可用廣東話（如「你發現咗一個新盲點💡」），但解析層必須 100% 標準書面語／英文
- 每次 Edit/Write bank/content 檔案後，自動運行 `npm run term-guard --silent`

### 學科術語強制對照表

| 英文術語 | 強制譯法 | 嚴禁用法 |
|---|---|---|
| Public Good | 共用品 | 公共財 |
| Entrepreneurship | 企業家職能 | 企業（單用）|
| Income Elasticity | — | **絕對禁止**（大學超綱）|

- 中文科 12 篇範文必須跨篇組合出題，設置考評局級別陷阱（過度推論、半對半錯、正確、無關）
- 數學科動態參數化 correct-by-construction，解析強制附帶 Casio fx-50FH II / 3650P 計數機 Program 教學
- 英文科 Paper 2 嚴格執行 Tone & Register 雙重語義批改，Paper 3 嚴打 Blind Copying 同 Over-paraphrasing

---

## 6. Validation Gates（數據變更前強制步驟）

**新增或收緊任何 schema/format validation gate 前，必須先查詢影響範圍。**

1. 統計現有幾多 published questions、registered topics、pending admin drafts 會 fail 新 gate
2. 打印 10 個例子
3. 如有影響，必須附 migration plan，**絕對禁止以 feature change 為由令現有數據集失效**
4. 題目 JSON 格式：`correctIndex`（0-3），**非字母式**；選項會洗牌；解析引內容不引字母

---

## 7. 大愛設計與 UDL 紅線

**絕對禁止出現打擊自信嘅元素：**

- ❌ 大紅交叉、❌「FAIL」字眼
- ✅ 改為「再諗下💡」或「你發現咗一個新盲點！」
- ⏸️ **30 秒逆向鎖死引擎 —— 2026-09-09 起暫停，見 §7.2。** 三維自診（概念盲區／審題陷阱／運算粗心）**維持生效**，剷走嘅只係「強制凍結界面 30 秒」呢一步。
  - 以下原條文**存檔保留**，因為兩個月後可能復活；刪走咗就冇人知呢個機制存在過、點解存在過：
  - ~~**30 秒**逆向鎖死引擎：**答錯任何一題**（唔再限中高難度）強制凍結界面 30 秒，逼使學生從「概念盲區／審題陷阱／運算粗心」三維自診~~
  - **2026-09-05 Brian 裁決修訂**（原條文：「答錯中高難度題…60 秒」）。
  - **點解由 hard-only 放寬到全部答錯題：** 發現引擎（BUILD-SPEC 功能一）靠呢一步入料 —— 學生每次自診完會記低一條「發現」，一節結尾喺 `/result` 見到「今日你發現咗 N 樣嘢」。按 3:5:2 出卷，一節 10 題只有 2 題 hard，答錯其中一半即係**一節得約 1 個發現**，而規格畫嘅係 3 個。一張長期只有一行嘅發現卡，證明唔到「你今日搞明白咗幾樣嘢」。
  - **點解同時由 60 減到 30：** 觸發次數上升，時長就要落返 —— 一節總凍結時間大致不變，唔會由「一次 60 秒」變成「四次 60 秒」。
  - **呢個係下限，唔係固定時長。** 學生一揀咗維度即刻可以睇解析；倒數只服務「唔揀嗰批」。冇下限的話個畫面會變成一個要撳走嘅彈窗。
  - 數值單一來源：`lib/discovery/dimensions.ts` 之 `REFLECTION_SECONDS`。`PracticeSession` 由嗰度 import，兩處各寫一個數字遲早會分叉。

### 7.1 一節練習題數 —— 2026-09-09 由 20 減到 10

**決策：** Yuna（COO）2026-09-09。單一來源：`lib/entitlements.ts` 之 `SESSION_SIZE`。

**依據（實測，唔係估）：**

| 數字 | 值 |
|---|---|
| 484 節入面冇做夠 20 題 | **357 節（74%）** |
| 每節題數中位數 | **15** |
| 174 個帳號做過至少一節 | **97（56%）** |

**假設：**「20 題」呢個數字本身就係一道門檻。**目標：中途離開率 74% → 30%，
2026-10-09 覆檢。** ⚠️ 呢個係假設，唔係已證實嘅結論 —— 覆檢要對返數，唔啱要改返。

**ADHD 考量：** 10 題 × 約 1.5 分鐘 ≈ 15 分鐘，坐得穩喺一個番茄鐘（25 分鐘）
之內，仲有位畀答錯後嗰 30 秒反思鎖。想做多啲就再開一節 ——
**十題十題咁加上去，唔係一開波就擺 20 題喺佢面前。**

⚠️ **§7 上文一直寫住「按 3:5:2 出卷，一節 10 題只有 2 題 hard」——
條文由頭到尾假設咗 10，係代碼嗰邊寫住 20。今次改完先至對得返。**
§12「每 session 20 題」講嘅係【出題入庫】批次，唔係練習卷長度，不受影響。

### 7.2 反思鎖剷除 —— 2026-09-09 起，為期兩個月

**決策：** Yuna（COO）2026-09-09。**性質：實驗，唔係永久裁決。**

**Yuna 原話：** 「因為要鎖 30 秒，對於學生嚟講真係幾難先可以真正學返，
所以先剷咗成個鎖……睇兩個月，睇下整體同平均正確率會唔會有變化。」

#### 剷咗啲乜、留低啲乜（呢個分別係實驗成立嘅關鍵）

| | 狀態 |
|---|---|
| 30 秒倒數凍結 | ❌ 剷 |
| 追問題（答啱先解得到鎖） | ❌ 剷 |
| server 簽名鎖 token（防 DevTools 篡改倒數） | ❌ 剷 |
| 鎖尾 5 秒提示音、沙漏動畫 | ❌ 剷 |
| **三維錯因一撳（概念盲區／審題陷阱／運算粗心）** | ✅ **留** |
| **`dse_reverse_log` ＋ `dse_discoveries` 寫入** | ✅ **留** |

**點解唔連自診一齊剷：** 撳錯因係【一下撳】，唔係一個鎖 —— 佢唔屬於 Yuna
所指嘅「難學返」嗰件事。而佢係錯題 DNA 雷達、遺忘曲線重溫、溫書地圖報告嘅
**唯一入料口**，亦係 §16.E（2026-09-08 雙簽、Brian 2026-09-09 副署）
特登開放上雲嗰個 key。

**更重要嘅係實驗本身：** 兩樣一齊剷，兩個月後正確率變咗，就分唔清係
「冇咗強制等待」定係「冇咗自我診斷」造成。只剷凍結，個變數先至乾淨。

#### 已刪檔案（git revert 即復活）

`components/HourglassTimer.tsx`、`lib/lockChime.ts`、`lib/lockoutQuestions.ts`、
`lib/lockout/{client,serverToken}.ts`、`app/api/lockout/route.ts`

Yuna 講明「如果之後覺得需要復活，我哋會再自己砌返出嚟」—— 實際上唔使砌，
git 歷史完整保留，`git revert` 就得。

#### 兩個月之後要交嘅嘢（2026-11-09 覆檢）

1. **逐週正確率 curve** —— 每星期答啱幾多條、答錯幾多條、正確率。
   資料源：`user_progress.progress_data → dse_progress` 之 `score` / `total` / `timestamp`
   （呢條路徑同個鎖完全無關，所以剷咗鎖照計得到）。
2. 對照基準（剷除前實測）：**整體正確率 88.3%**、5,565 題、484 節、中位 15 題。
3. 睇完 curve 先決定：復活個鎖、永久刪除、定係改個形態。

⚠️ **實驗期間唔准靜靜哋接返個鎖。** 迴歸鎖：
`lib/__tests__/discovery-local-only.test.mts` 測試 ③ 會掃 `PracticeSession.tsx`，
見到 `LOCKOUT_SECONDS`／`lockDeadlineRef`／`pickLockoutQuestion` 等即刻 fail。
測試 ④ 同時守住「自診唔准一齊剷走」。

#### 一個順帶嘅缺口 —— 2026-09-11 已補

`dse_calm_lock`（柔和呈現偏好）**唯一嘅開關**本來就喺鎖嗰個介面入面。
鎖剷咗，個掣一齊冇咗 —— 個設定仲喺度、仲會同步、仲影響指令字高亮嘅呈現，
但學生改唔到。**已搬入 `A11yPanel`**（同其餘 SEN 控制擺埋一齊），
標題「柔和呈現」，帶 `aria-pressed`。

**刻意冇納入「一鍵舒適模式」嘅推導** —— 納入嘅話，今日已經開住舒適模式嘅學生
會因為 `calm` 仲係 0 而突然見到總掣變「關」。一個純粹補返個掣嘅改動，
唔應該令任何人已有嘅狀態顯示變樣。

**迴歸鎖：`lib/__tests__/settings-have-controls.test.mts`。**
佢守嘅唔係嗰一個掣，係【呢一類 bug】—— 由 `lib/settingsSync.ts` 自己攞晒
上雲嘅 key，逐個檢查 `components/` 同 `app/` 入面有冇地方寫得入。
一個會同步但改唔到嘅設定唔會嗌，亦唔會令任何測試紅，佢淨係靜靜哋卡死喺某個值；
對一個真係需要柔和呈現嘅 SEN 學生嚟講，佢見到嘅係「呢個功能唔存在」。

⚠️ **實作途中踩到一個掃原始碼測試捉唔到嘅 bug：** 副作用（寫 localStorage ＋
派 `dse-a11y`）本來擺咗喺 `setCalm` 個 updater 入面，而新加嘅 `dse-a11y`
listener 又會 `setCalm` —— 變成 re-entrant，撳完個掣彈返原位。
四條測試全綠，個掣係壞嘅；係喺 `localhost:3001` 撳落去先發現（實測另外兩個掣
0 → 1，呢個 0 → 0）。**憲章 §4 要求喺運行中 UI 手動驗證，就係為咗呢種。**
- ✅ 無痕 SEN 專注模式（OpenDyslexic 字體、防跳行閱讀尺、隱藏倒數計時器）
- ✅ 考前深呼吸急救室（「我好緊張」按鈕觸發 1 分鐘 4-7-8 呼吸法動畫）
- ✅ 隱藏天賦雷達圖（放棄單一總分，顯示空間幾何直覺等隱藏天賦）

**SEN 功能狀態：全部上線**（OpenDyslexic 字體、防跳行閱讀尺、柔和呈現、專注燈、休息日護盾、呼吸急救室）。每項都有迴歸測試鎖住，改壞咗會喺 CI 紅。
（2026-09-26 刪去原文「零故障保證」：冇 runtime 監測就唔可以講「零故障」（§16.D）。Yuna 2026-09-21 同意，按 §18 生效；見 `docs/charter-inconsistency-draft-2026-09-16.md` 第三項。）

---

## 8. 已否決項目永久攔截（2026-08-22 更新）

以下項目已被徹底否決，**復建 = 不合格**：

- JUPAS 預測器 / admission probability
- **收費模式 / Premium / 訂閱 / 付費牆 / 廣告**（2026-09-05 恢復否決 —— 2026-09-04 曾短暫解禁，見 §8.2）
- 老師平台 / IEP 草案（classes/enrollments/question_events 已刪）
- 虛構統計（Band A+5% / 轉化率 / 假在線人數）
- **同學之間的比較**：「同級 N 人平均」、百分位、排名、匿名同儕對比，一律不做（2026-09-26 Yuna 重申：嚴禁改做「同級 100 人平均」；見 §16.E 約束 3）
- 假用戶見證
- 舊名 Aethel / WisdomPath / 化城避風港 / Buff 補給艙
- **長答／作文自動批改、打分、評分**（2026-08-25 新增，詳見 §16.A）
- **向未成年人隱瞞 AI 身份**（2026-08-25 新增，詳見 §16.B）
- **預填驗收表**（2026-08-25 新增，詳見 §16.C）
- **「醫療級」字眼／將 build-time 測試講成 runtime 防護**（2026-08-25 新增，詳見 §16.D）
- **瀏覽器端大語言模型**（Qwen 1.5B／每人 600MB 下載）—— 2026-08-25 廢止，改行零模型檢索版

### 8.1 Gamification —— 2026-08-22 解禁

**舊條文（2026-07-16）：**「Gamification（EXP / 段位 / combo / 火焰 / 解鎖機制）」列為永久否決。
**現行條文：** 該項否決**已撤銷**。連擊、EXP、段位、火焰、Boss 模式、解鎖機制、
虛擬貨幣（減壓緩衝區內）**准許製作**。

- **決策：** Brian（CEO）拍板，Yuna（COO）於 2026-08-22 傳達並指示執行。
- **依據文件：**《DSE_Level_Up UI／UX 整改與優化方案（Originkit Edition）》
  v2026-08-22-FINAL，§0 列明「遊戲化全家桶（Combo、XP、Boss、Rank、虛擬貨幣）
  經 CEO 拍板保留」。
- **點解要寫低：** 舊條文同新指令直接對撞。唔喺憲章度調和，每個新 session 都會
  按 §8 攔截遊戲化需求一次 —— 呢個模式（憲章自己同自己打架）過往已重複出現，
  每次都要重新爭論一輪。寫低咗，往後照憲章行事即可。

**解禁之後仍然生效嘅約束（呢幾條冇變）：**

1. **$0 成本**（§5）—— 遊戲化唔可以引入任何付費 API、新套件或雲端用量。
2. **大愛設計紅線**（§7）—— 唔准出現打擊自信嘅元素。連擊中斷、Boss 反擊、
   排名下跌一律唔可以用懲罰性回饋（震屏、紅叉、FAIL 字眼、負面音效）。
   答錯嘅回饋力度必須同答啱一致。
3. **唔可以侵蝕練習流程**（2026-08-22 Yuna 指示）—— 遊戲化層要同 `/practice`
   嘅核心作答流程解耦；練習頁本身嘅節奏、專注度同 SEN 支援優先於遊戲化。
4. **SEN 必須可以整層關掉**（§7）—— 一鍵舒適模式之下，連擊、火焰、粒子、
   成就彈窗全部隱藏，唔係調慢。
5. **虛擬貨幣只限減壓緩衝區內部，嚴禁真實貨幣交易、嚴禁換取任何學習特權**
   （免費原則 §3：遊戲化唔可以變相成為付費牆）。
6. **唔准虛構數據**（§8）—— 排行榜、在線人數、他人成績一律唔可以作假；
   冇真實數據支撐嘅比較功能寧可唔做。

**不恢復（2026-09-26 Yuna 重申）：** 每日連續（「連續 N 日」）維持移除 —— 2026-08-23 HOTFIX-0823
已改為累積日數（`lib/logicLog.ts`、`lib/progress.ts`，`no-daily-streak.test.mts` 鎖住）。
本節文字維持不變；一節之內的連擊（combo）不受影響。

**允許（一直有效）：**「減壓緩衝區」放鬆功能（呼吸動畫、金句輪播、休息按鈕），
必須 $0 成本。放鬆功能本身唔可以被遊戲化任務取代 —— 學生想淨係抖吓，
要照樣抖得到。

### 8.1.1 房間建造／虛擬超市 —— 2026-09-05 剷除

**原條文寫住**「減壓緩衝區可以加入**房間建造、虛擬貨幣**等遊戲化元素」。
按此起過兩個功能，2026-09-05 Brian 裁決全部剷除：

- **邏輯家園**（`/homestead`，SPEC-GAMIFY-P1-20260822 §模組二）—— 四區等級由
  現有數據衍生嘅動物森友會式家居系統
- **虛擬超市**（`/relax/virtual-supermarket`）—— iframe 載入嘅零依賴靜態超市

**已刪：** 8 個位置、27 個檔、3,023 行（含 `public/supermarket/` 20 個靜態檔）。
入口亦一併清走：`sitemap.ts`、`RelaxLanding` 嗰張卡、`TrailStrip` 嗰條家園連結。
**冇任何資料表、冇任何 localStorage key 需要清** —— 家園等級一直係衍生值
（`lib/homestead.ts` 檔頭明寫 `dse_homestead_state` 同 `dse_homestead_cache`
【不會被寫入】），超市亦冇存過任何嘢。

**⚠️ 呢節【刻意保留】而唔係將原條文嗰句刪走算數。** 同 §8.2 一樣嘅理由：
一句仍然寫住「可以加入房間建造」嘅准許條文，落一個 session 就會被讀成待辦，
然後再起一次。§8.1 自己開頭嗰段講嘅正正就係呢個模式。

**§8.1 其餘部分不受影響** —— 連擊、EXP、段位、火焰、Boss 模式、解鎖機制
維持解禁；六條約束維持生效。剷嘅係「房間建造 ＋ 虛擬貨幣」呢一路，
唔係整個遊戲化。

### 8.2 受控收費框架 —— 2026-09-04 新增，2026-09-05 撤銷

**現行條文：整套收費框架已撤銷。** 收費模式恢復為 §8 永久否決項目。

- **決策：** Brian（CEO）2026-09-05 裁決：「Delete 晒成個 payment 系統，然後將佢哋所有關於 payment 嘅頁面鏟除，照舊完全免費俾佢哋做無限題目。」
- **執行：** 代碼、頁面、API、五張資料表、`stripe` 套件、budget-guard 例外，全部同日剷除。

**⚠️ 呢節【刻意保留】而唔係整段刪走。** 一節寫住「已撤銷」嘅條文，同一節唔存在嘅條文，對下一個 session 嚟講唔同：前者答咗「收費係咪諗過？」（諗過，起過，剷咗），後者會令同一個提案以「新諗頭」姿態再嚟一次。§16.A 同 §16.E 都係靠呢個做法攔住重複提案。

**曾經存在過嘅嘢（存檔）：** 5 個 SKU 白名單（Plus 月費 $28／考季通行證 $168／年費 $198／單科試卷組 $38／衝刺包 $88）、2028-03-31 前 100 筆交易嘅驗證目標、連續 2 個月不達標即停嘅熔斷機制。**一筆都冇成交過** —— 系統喺暗部署階段就被剷除。


---

## 9. 品牌語調與金句

**語調：** 學長學姐式共情力與熱血感，絕對唔准冷冰冰企業語氣或過度學術化腔調。

**核心金句（必須熟悉）：**
- 「最後 30 日，唔係溫書，係搶分」
- 「掌握邏輯，唔係背答案。無論數字點變，你都識答」
- 「thinka 幫你學三年，DSE Level Up 幫你贏最後三十分」
- 「Free users = mission. Revenue sustains the mission; the mission is not revenue.」
- 「慳返幾千蚊補習費，攞分嘅真功夫唔應該被窮富擋住」

---

## 10. 三關審核準則（每次輸出前強制自檢）

1. **處事層面：** 有遠見（掌握長遠趨勢）× 精準部署（每步鉅細無遺）× 細節控（學生感受得到用心）
2. **心態層面：** 熱血 Aggressive（加快成功）× 冷靜沉穩（機械人式思考，情緒不主導判斷）
3. **思考層面：** 反思對手成功哲學嘅底層邏輯，將之轉化為自身武器

---

## 11. Kimi 慣性錯誤防範清單（出 spec 前必對照）

- [ ] 無假設 Drizzle / Better Auth / Supabase Auth / middleware.ts
- [ ] 無出 SQL 動已刪／從未存在表
- [ ] correct_answer 用 `correctIndex`（0-3），非字母式
- [ ] 解析用標準書面語／英文，term-guard 攔截口語
- [ ] 無虛構統計／見證／人數
- [ ] 無復用舊名／舊功能（⚠️ gamification 已於 2026-08-22 解禁，見 §8.1，唔屬此列）
- [ ] **無任何收費／訂閱／付費牆／廣告提案**（§8 恢復否決，§8.2 已撤銷）
- [ ] **無新增付費套件或 SaaS**（§5；Stripe 例外已收回，`npm i stripe` 會被 budget-guard 攔）
- [ ] 無預填驗收表
- [ ] SEN 功能已上線，唔使再講「未上線」
- [ ] **無長答／作文自動批改、打分、評分**（§16.A —— 已復現 20 次，最高頻嘅一項）
- [ ] **AI 功能唔會隱瞞自己係 AI**（§16.B）
- [ ] **驗收表「最終狀態」全部留空，等實跑**（§16.C）
- [ ] **冇「醫療級」，冇將 build-time 測試講成 runtime 防護**（§16.D）
- [ ] **冇 `user_id` ＋ 課題 ＋ 正確率 嘅 schema**（§16.E —— 已復現 7 次）

---

## 12. 生產紀律（題目入庫流程）

**決定：** Yuna（COO）2026-09-26，按 §18：「想要『自動入 approved』。」新題過機器閘即上線，毋須事前真人審批。

### 12.1 自動入庫（預設通道）

```
drafts/
  ↓ scripts/qbank/auto-promote.mts（機器閘：格式、四個相異選項、術語紅線、
  ↓   與現有題庫的重複度、批次內重複、課題 id 對得上）
  ↓ data/questions/*-auto.ts（framework = auto，「機器閘放行題」）
  ↓ npm test ＋ npm run gen:summary
  ↓ 部署，再 sync-questions --push
```

約束（全部生效）：

1. **照實披露。** 題目出處欄寫「經自動檢查」，並說明未有真人逐題審批（`components/QuestionProvenance.tsx`、`/transparency`）。
   不得移除、摺埋或淡化這個披露（§8、§16.D）。
2. **機器驗不到答案在學術上是否正確。** 草稿必須 correct-by-construction（數理科由程式計出），或引用可查證的原文；機器閘只是最後一道濾網，不是品質來源。
3. **事後覆核及退回。** 創辦人發現問題，用 `npx tsx scripts/qbank/withdraw.mts --subject <科目> --id <題號> --reason "<原因>"` 退回。
   退回寫入 `data/questions/withdrawn.json`，記錄日期及原因；題目檔不刪，可用 `--undo` 撤銷。退回後須 gen:summary、部署及 sync-questions --push。
4. **§16.A 不受影響。** 本節講的是題目上線，不是批改學生答案；長答自動批改維持永久禁止。
5. 自動通道由 2026-08-22 Yuna 指令建立（`auto-promote.mts`），但本節舊文一直寫「機器永不自動入庫」，兩者並存約一個月而互相矛盾。
   2026-09-26 起以本節為準；當日已有 728 條機器閘題在線。

### 12.2 人手審批通道（保留，可選）

```
drafts/
  ↓ review-drafts.mjs（自動格式檢查）
  ↓ 真人逐題批（學科首席把關）
  ↓ promote-drafts.mjs（符合 JSON schema）
  ↓ decisions.json（審批記錄；2026-09-26 起毋須簽名，見 §18）
  ↓ 人手 wire 入 load.ts
  ↓ npm test（驗證通過）
```

### 12.3 舊條文（存檔）

~~機器永不自動入庫。~~（2026-09-26 由 §12.1 取代。保留此句，是為了讓下一個 session 知道這條線動過、由誰決定。）

**每 session 20 題，DIFF_RATIO 3:5:2（basic:intermediate:hard）。**

---

## 13. 免責聲明（每頁必須）

> 「本平台提供之試題均為獨立改寫版本，旨在協助考生練習應試技巧，並非香港考試及評核局（HKEAA）官方試題。官方歷屆試題請前往 HKEAA 網站下載。等級預測僅供參考，最終成績以 HKEAA 公布為準。」

---

## 14. 顏色系統參考（UI/UX）

- **預設淺色莫蘭迪**：頁面底 `#F4F0EA`、正文 `#2C2A29`、強調 `#57685C`（sage）。
  色值單一來源係 `app/globals.css`，對照表見 `docs/tokens.md`；憲章唔再各自抄一份 hex。
- **暗色主題 `cyber`**：`#1a1917` 底 ＋ sage 綠，非藍底青光。
- **霓虹四色（`--color-neon-*`）只保留喺導出圖卡**（溫書地圖 PNG、成績卡）——
  嗰批圖要離開網站都睇得明，所以鎖定標準色版，唔跟主題變數。
- WCAG AA 由 `scripts/contrast-guard.mjs`（掃源碼）＋ `scripts/a11y/runtime-probe.js`（量實際渲染）守。
- 字體：Display 20pt、H1 14pt、H2 11pt、Body 9pt、Caption 7pt
- （2026-09-26 改寫。原文「暗色護眼背景 ＋ 霓虹強調色」「Cyber-Academic 暗底霓虹戰鬥風」同實際網站不符。
  Yuna 2026-09-21 同意對齊實情，按 §18 生效；色值用 2026-09-23 更正版，
  見 `docs/charter-inconsistency-draft-2026-09-16.md` 第二項。）

---

## 15. Testing 強制條文

修改題庫、topic registration、或任何模組後：
1. 運行 `npm test` 並報告 pass/fail count
2. **Tests + Working Integration 雙重 required——單獨一樣唔算完成**
3. 如有 fail，先修復，後報告

---

## 16. A／B／C／D／E 五條永久紅線（2026-08-25 停火令；2026-09-04 標題更正 ＋ 16.E 修訂）

**來源：** 2026-08-25 22:23 最高指揮部緊急叫停令，Brian（CEO）簽署、Yuna（COO）傳達。
**適用：** 此後**任何**提案自動適用，**不經討論，直接拒絕**，唔使逐次重新爭論。

---

### 16.A 長答自動批改 —— 永久禁令

> 任何提案出現「**批改／打分／評分**」＋「**長答題／作文／Paper 2／Paper 3**」嘅組合，
> **一律預設拒絕。**

**點解要寫低：** 呢個要求**反覆復現** —— 每一版憲章、規格書、召集令都會重新引入一次，
而且措辭逐字重複（「Tone & Register 雙重語義批改」「嚴打 Blind Copying 同 Over-paraphrasing」
「C/L/O 三維分數」「必中關鍵字同加分關鍵字」）。
唔喺憲章度一次過封死，每個新 session 都要重新爭論一輪。

**復現次數（長期日誌逐次記錄）：** 編號截至 2026-08-20 已到**第 18 次**，
加埋 2026-08-27 兩份停止備忘（SEN 150 項擴展、SENSEI 召集令），**已達 20 次**。

⚠️ 停火令原稿寫「第九次」—— 嗰個係**單一 session 內**嘅計數，並非全紀錄。
真實數字高一倍有多。喺呢度寫低，係因為低估次數會令人以為係偶發疏忽，
但實情係一個**系統性、跨作者、跨文件類型**嘅模式：
出現過嘅載體包括憲章、規格書、skill 定義、我自己嘅 To-Do 清單（改寫後交返）、
題目 JSON schema 本身，甚至一份「自動批改課程」嘅賣點。

#### ⚠️ 現行機制實情（2026-08-25 逐檔核實 —— 與停火令原稿有出入，**以本節為準**）

停火令原稿寫：「現有 Regex 引擎只限客觀題（選擇題、填充題、短答題）」。
**實測：本項目根本冇任何 Regex 批改引擎**，而且填充題／長題**亦唔係機器批改**。

| 題型 | 型別定義 | 實際批改方式 |
|---|---|---|
| `mc` 選擇題 | `data/questions/types.ts:5` | `PracticeSession.tsx:433` `zh === currentQ.correctZh`（`correctZh` 由 `correctIndex` 取得，line 114）—— **唯一由機器判對錯嘅題型** |
| `text` 填充／短答 | `types.ts:43` | `referenceAnswer` 交卷後顯示，**學生自己對照自評**（`TextQuestionCard.tsx:10`） |
| `long` 長題／作文 | `types.ts:68` | 三級自評（全明／部分明白／未明），原始碼註釋明寫 **Never auto-graded**（`LongQuestionCard.tsx:13`） |

核實指令（可自行重跑）：

```
grep -rniE "acceptedAnswers|gradeText|checkText|answerKeywords|rubricMatch" lib/ components/ app/
→ 全 repo 零命中
```

（`keyword` 嘅命中全部係畀學生睇嘅提示文案，例如 `lib/lockoutQuestions.ts:65`
「circle the keywords」，唔係評分邏輯。）

#### 正確條文

> **機器只為 `mc` 出對錯。`text` 同 `long` 平台一分都唔出** ——
> 只提供參考答案／評分準則畀學生自評。

呢個比停火令原稿**更嚴**，而且係現況。

⚠️ **點解唔可以照抄原稿嗰句：**「Regex 引擎可批改填充題、短答題」寫入憲章之後，
會被下一個 session 讀成**授權**，然後真係去起一個 —— 一條用嚟收窄嘅紅線
反而變成放寬。憲章入面嘅「現況描述」如果同實情唔符，殺傷力同假需求一樣大。

#### 結構性防護（唔准拆）

`StepHints` 嘅唯一 prop 係 `subjectId` —— 佢**收唔到學生寫嘅嘢**，
所以長答自動批改喺物理上唔可能發生。呢個唔係巧合，係設計。

---

### 16.B AI 身份誠實義務

> SENSEI 或任何其他 AI 功能，**可以**採用師兄師姐語氣，
> 但**被問及是否真人時必須如實回答**。
> **絕對禁止向未成年人隱瞞 AI 身份。**

#### 標準回答（停火令核准，逐字）

> 「Sensei 係 DSE LEVEL UP 嘅 AI 學習助手，設計成師兄師姐嘅語氣陪你溫書。
> 雖然 Sensei 唔係真人，但背後嘅知識同建議全部係由考過 DSE 嘅團隊審核過嘅。」

#### 點解

用戶群係 **12–18 歲**。一個中六生半夜三點同「師兄」傾偈，
以為對面係真人嗰陣**投放嘅信任**，同佢知道係 AI 完全唔同。
再配合情緒語氣適配，一個情緒低落嘅學生會將佢當成情感支持。

呢條同 §8 禁「假用戶見證」「虛構 persona 當真人」係同一條原則 ——
只不過今次個虛構對象係 24 小時貼身嘅。

**語氣同誠實唔矛盾。** 親切、唔端架子、講返自己點衰過 —— 全部照做得。

#### 執行要求

1. 介面永遠有一個**唔遮得住**嘅 AI 標示（唔可以摺埋、唔可以只喺首次出現）
2. 「你係咪真人／你係咪 AI／你係唔係機械人」必須觸發如實回答 ——
   唔可以繞開、唔可以打哈哈、唔可以反問
3. 呢個判定必須有測試鎖住，唔可以只靠 prompt

---

### 16.C 預填驗收表 —— 即時停火

> 驗收表任何「最終狀態」欄位**必須留空**，直到有人**實際跑過**。
> **「冇人跑就填」呢個動作本身就係違規，不論數字最終啱唔啱。**

**觸發即停火，唔使爭論。** 適用於：QA 閘表、測試 pass/fail 數、tsc／eslint 狀態、
build 結果、「已上線／已完成」欄位。

**點解要寫「不論數字啱唔啱」：** 2026-08-25 嗰張七閘表最後驗出嚟七項全部真係綠 ——
但佢係**填先、跑後**。今次啱，下次唔啱嗰陣一樣會綠，而且冇人會再核。
一張填得出「✅ 真實閘：有無假陳述？通過」而冇人跑過嘅表，本身就係佢要捉嘅嘢。

**執行：**
1. 虛擬 persona **唔可以做驗收人**（Kelly／Ghost／Plaaaaaa 等只提供檢查清單）
2. 數字由實跑者填，並附可重跑嘅指令
3. 未跑嘅一律寫「⬜ 待驗證」，唔准寫 ✅

---

### 16.D 禁止 runtime／醫療防護聲稱

> 唔准用「**醫療級**」三個字。
> 唔准將 **build-time 測試**講成 **runtime 防護**。

**判例（2026-08-25，光敏感）：**

| 寫過 | 實情 |
|---|---|
| 「**自動攔截**超過 3Hz 嘅閃爍動畫，癲癇／光敏感學生可以**安心溫書**」 | 3Hz 閘係 `lib/__tests__/sen-accessibility.test.mts:321` 一個 **build-time 測試**，掃 `globals.css` 然後 fail build。**runtime 攔截 = 零** |

**點解呢項比一般誇大嚴重：** 一個光敏感學生睇到「有自動攔截」，可能因此
**唔開自己部機或瀏覽器嘅光敏防護**。呢個唔止係誇大，係一個**會改變學生安全行為**
嘅假聲稱。

**正確講法（准用）：**

> 「我哋自己嘅動畫全部低於 3Hz，而且有測試鎖住，唔會有一日改壞咗都冇人知。」

呢句已經夠好，而且係真。

**通用規則：** 描述任何安全／無障礙功能之前，先答一條問題 ——
**「呢樣嘢喺學生部機上運行緊，定係喺我哋部 CI 度運行緊？」**
答案係後者，就唔可以用保護學生嘅語氣去講。

---

---

### 16.E 個人逐課題正確率 —— 限用戶本人查閱之上雲（2026-09-04 修訂）

**舊條文（2026-08-25，已廢止）：**「任何含『`user_id` ＋ 課題 ＋ 正確率／錯誤次數』
組合嘅 schema 提案，一律駁回，唔需要再評估。」

**現行條文（2026-09-04 生效）：**

> 逐課題正確率（`dse_topic_stats`）**可**上雲儲存，用於跨裝置同步，
> 但受以下**全部**約束：
>
> 1. **只限用戶本人查閱。** 讀取必須經 Auth.js session 綁定 `user_id`，
>    server-side 核對；任何情況下不得返回他人數據。
> 2. **禁止向任何第三方提供** —— 包括但不限於學校、老師、補習社、家長、
>    研究機構、廣告商。無論匿名化與否、無論收費與否。
> 3. **禁止跨用戶匯總比較。** 不得據此製作排行榜、班級平均、同儕對比、百分位排名。
>    （承 §8「唔准虛構數據」及 §8.1 約束 6：「冇真實數據支撐嘅比較功能寧可唔做」
>    —— 有真實數據亦唔做跨用戶比較。）
> 4. **不得成為 Plus 付費牆。** 用戶本人查閱自己嘅逐課題正確率，
>    喺免費層必須完整可見；Plus 只可以加快、加靚呈現，**唔可以遮蔽數據本身**。
> 5. **答案原文准許上雲，僅限跨裝置連續性所需之兩個 key**（2026-09-08 修訂）：
>    `dse_active_session`（未完成嗰節，含 `answers[].selectedZh`）及
>    `dse_reverse_log`（錯因自診，含 `selected` / `correct`）。
>
>    **界線：學生【揀】嘅嘢（選擇題選項）可以上雲；學生【寫】嘅嘢永遠唔可以** ——
>    情緒記錄、時間囊、作文草稿、心情備註、自寫鼓勵語**維持永久禁止**，
>    且不得援引本次修訂作先例。
>
>    ⚠️ **舊條文（2026-09-04）寫住「維持禁止上雲，本次修訂不涵蓋此項」，已廢止。**
>    保留呢句喺度，係因為刪走咗，下一個 session 就唔知呢條線郁過 ——
>    而佢郁過，係經過開題、選項比較、簽名同測試，唔係隨手鬆。
>    決策人：Yuna（COO）2026-09-08，Brian（CEO）2026-09-09 副署 —— **雙簽完成，正式生效**。
>    全文：`docs/charter-amendment-2026-09-08.md`。
> 6. **上雲白名單仍由 `lib/sync.ts` 定義，新增任何一個 key 仍須創辦人明確決定。**
>    2026-09-04 修訂構成 `dse_topic_stats` 一個 key 之批准；
>    2026-09-08 修訂構成 `dse_active_session` ＋ `dse_reverse_log` 兩個 key 之批准。
>    **合共三個 key，不延伸至任何其他 key。** 本條本身永不因批准過幾多個而放寬 ——
>    第四個 key 一樣要重新開題、重新決定。
>    （2026-09-26 起「書面批准」改為創辦人明確決定並記錄於 `docs/`，毋須簽名，見 §18。）
>    **2026-09-26：第四個 key `dse_electives`（每科選修單元選擇）** 經開題
>    （`docs/charter-amendment-2026-09-26-electives-DRAFT.md`）後由 Yuna 決定上雲，
>    經 `lib/sync.ts` 同一通道，唔開新表。屬學生【揀】嘅嘢，符合約束 5 界線。
>    本條其餘內容不變：第五個 key 一樣要重新開題、重新決定。
> 7. **老師平台維持已否決狀態**（§8）。本次修訂**不**構成復建理由 ——
>    `classes` / `enrollments` / `question_events` 已於 `0003_drop_teacher_platform.sql`
>    刪除，維持刪除。

**決策：** Brian（CEO）2026-09-04 就本條**單獨開題**並裁決（符合舊條文執行第 4 點
「由創辦人主動開題，唔接受夾喺其他提案入面順帶提出」），Brian ＋ Yuna 雙簽生效。
**依據文件：** `docs/charter-amendment-2026-09-04.md`。

**修訂前狀態（存檔）：** commit `9adc536`（2026-08-24）曾將 `dse_topic_stats` 同
`dse_active_session` 由上雲白名單剔走，`lib/sync.ts` 只上傳 `dse_progress` 同
`dse_free_attempts_total`。本次修訂只放行前者，後者維持剔走。

#### 點解原禁令要寫入憲章（保留存檔 —— 解禁後嘅風險同源）

呢個提案到 2026-08-27 已經出現咗**第七次**，載體包括：規格書 v2.0／v2.1、
2028 戰備手冊、業務架構規格 v1.0、《全員優化建議書》E-02。

**復現嘅原因唔係大意，係結構性：** 錯題 DNA 要有**課題粒度**先至有用。
任何人認真設計「錯題 DNA 跨機同步」，都會由第一原理推導出同一張表。
所以佢每次都會「新鮮咁」出現，而每次提出嘅人都覺得自己諗到一樣好嘢 ——
E-02 甚至自己標咗「風險：高，要創辦人親自簽名」，程序上企理。

但一個**已經關咗嘅門**，唔應該每次都重新包裝成一條等人開嘅門。
提案階段冇人會去睇 `lib/sync.ts` 嘅 git log，所以裁決寫喺 commit message
入面等於冇寫。要攔就要攔喺每個 session 一定會讀到嘅位置。

#### 解禁之後嘅新風險（2026-09-04 新增）

呢張表一旦上雲，下一步提案好可能係「畀老師睇埋」「做班級報告」「同其他學生比較」
「Plus 先睇到全課題」—— 呢四樣分別由上文約束 2、7、3、4 明文禁止。
上面嗰段結構性觀察（錯題 DNA 要有課題粒度先有用，任何人認真設計都會推導到同一張表）
**解禁之後依然適用**，只係推導終點由「一張表」變成「一個更闊嘅讀取權限」。

#### 執行

1. 上雲白名單由 `lib/sync.ts` 定義。**新增任何一個 key 都要創辦人明確決定**（記錄於 `docs/`，毋須簽名，見 §18）。
   已批准：`dse_progress`、`dse_free_attempts_total`、`dse_topic_stats`（2026-09-04）、
   `dse_active_session`、`dse_reverse_log`（2026-09-08）、`dse_electives`（2026-09-26，Yuna，按 §18）。
   ⚠️ `dse_topic_stats` 2026-09-04 批准咗，但代碼直到 **2026-09-07** 先跟上 ——
   中間三日條文同實作漂移咗，學生換部機雷達圖照樣由零開始。
   **批准同落實係兩件事，兩件都要有人做。**
2. 收到「畀第三方睇」「跨用戶比較」「用嚟做付費牆」類提案：
   直接引用本節約束 2／3／4 駁回，**唔使逐次重新評估風險**。
3. **唔准提出「窄啲、應該過到關」嘅替代方案**去繞開約束 2、3、4。
4. 如日後要再檢視本裁決，由創辦人主動開題，唔接受夾喺其他提案入面順帶提出。

---

## 17. 社會回饋條款 —— 2026-09-04 新增，2026-09-05 隨收費一併凍結

**現行狀態：全六條凍結。** 原六條（無證明減免制、防濫用機制、不得製造申請壓力、
10% 淨收入捐款、每季 `/transparency` 公開收支、80/20 收入分配）**全部以收入存在為前提**。
收費系統已剷除，冇收入即冇 80/20 可分、冇 10% 可捐、冇收支可公開。

**點解要明文凍結，而唔係擺喺度當佢仲有效：** 一條兌現唔到嘅承諾寫喺憲章度，
同 §16.D 嗰句「自動攔截 3Hz 閃爍」係同一個病 —— 讀嘅人會以為有，實際上冇。
`scripts/claims-guard.mjs` 守嘅正正就係呢種對外宣稱。

**⚠️ 唯一一條唔靠收入、而且已經做到嘅嘢，維持有效：**
全部功能對所有學生免費、無限次，冇任何經濟門檻。§9 嗰句「慳返幾千蚊補習費，
攞分嘅真功夫唔應該被窮富擋住」——**冇咗收費之後，佢反而係無條件成立。**
原 §17.1 個「50% 減免」本身就係一個收費系統先需要嘅補丁；冇收費，就冇嘢要減免。

**日後若重啟收費**（須創辦人明確決定；2026-09-26 起毋須簽名，見 §18），本節六條一併重新生效並重新審視。

---

## 18. 取消簽署制度 —— 2026-09-26 起

**決定：** Yuna（COO）2026-09-26：「以後所有項目或者所有決定都不需要簽名。」

**現行條文：** 創辦人（Brian 或 Yuna）的指示即為決定，**即時生效**，毋須簽名、副署或具名審批紀錄。
本憲章其他各節凡寫「雙簽」「副署」「書面批准」「重新簽名」的**要求**，一律按本節理解為「創辦人明確決定」。
各節記錄過往某次由誰簽署的**歷史紀錄**原文保留，不作改寫。

### 18.1 仍然要做的事

1. **記錄決定。** 每項決定記錄決定人、日期、原話或摘要，寫入 commit message 或 `docs/`，並同步至 Notion（§19）。
   取消的是簽名，不是紀錄 —— 沒有紀錄，下一個 session 就不知道這條線動過（§8.1）。
2. **不代簽。** Claude 及任何虛擬 persona 不得代任何人簽名、剔選或填寫審批人。
   簽名取消後，這些欄位是「不再需要」，而不是「可以代填」。
3. **§16.C 不受影響。** 驗收表記錄的是事實，不是簽名：未實際執行的一律寫「⬜ 待驗證」，不得預填。
4. **§4 不受影響。** 超過 3 個檔案的改動仍先出影響報告，等創辦人一句「照做」。這是決定，不是簽名。
5. **§1 最高憲章及 §16 各條紅線的內容不變。** 本節只取消簽署程序，不放寬任何內容限制。

### 18.2 未簽署的草案

`docs/` 內標記「⬜ 未簽署」的草案，不再因欠簽名而擱置。但每份草案是否生效、
其中列明的未決問題如何處理，須由創辦人逐份決定，再寫入本憲章。
草案不會因本節而自動生效 —— 部分草案本身仍有未答的問題。

`docs/charter-amendment-2026-09-25-DRAFT.md`（取消實名審批程序）的問題一
（取消實名審批之後，「機器永不自動入庫」由甚麼守住）已於 2026-09-26 決定：
改為過機器閘即上線、事後覆核退回，見 §12。人手通道 `promote-drafts.mjs` 保留作可選，
現時仍要求審批人欄非空（`scripts/qbank/_reviewer-gate.mjs`）；用自動通道則不需要。

---

## 19. Notion —— Claude Code 的第二大腦（2026-09-26 起）

**決定：** Yuna（COO）2026-09-26：「新增憲章『Notion Connectors』將會成為 DSE Level Up Website Claude Code 嘅第二大腦。
佢會記憶住成個流程同埋做過嘅所有嘢。」

### 19.1 用途

Claude Code 透過 Notion connector，把本項目的**流程、決定和已完成的工作**記錄在創辦人的 Notion 工作區：

- 每個 session 開始時先讀 Notion，了解最近的決定、待辦事項和覆檢日期。
- 每個 session 結束前寫入：做了甚麼、改了哪些分支和 commit、測試結果、未完成事項、需要創辦人決定的問題。
- 創辦人的每項決定（§18.1 第 1 點）同步記錄。

### 19.2 地位：紀錄與索引，不是正本

Notion 與 repo 內容有衝突時，**以 repo 為準**：

| 內容 | 正本 |
|---|---|
| 憲章 | `docs/charter.md` |
| 題庫 | `data/questions/*.ts`（§3.1 約束 3） |
| 代碼及改動歷史 | git |

發現 Notion 與 repo 不符時，更正 Notion，不是反過來改 repo。
兩份文件各自演變而互相矛盾，正是 §8.1 記錄過的「憲章自己同自己打架」。

### 19.3 嚴禁寫入 Notion

1. 任何密鑰、token、`.env.local` 內容、OAuth client secret。
2. 學生個人資料：電郵、姓名、user_id、帳號，以及個別學生的作答、進度、錯因、情緒紀錄（§16.E）。
   只可寫入無法識別個人的合計數字。
3. Supabase 資料列原文。
4. 考評局或教育局受版權保護的原文（§1 第 4 點）；只可寫章節名稱、頁碼及連結。
5. 題目全文。題庫正本只在 repo；Notion 只記題目 id 和數量，不另建第二份題庫。

### 19.4 其他

- **網站本身不連接 Notion。** Notion 是開發用的紀錄工具，不屬網站技術棧；網站不新增套件，亦不調用 Notion API（§5）。
- 只使用現有 Notion 帳戶，不為此新增付費方案。
- Notion 頁面的分享範圍由創辦人控制。分享予他人前，須確認內容符合 19.3。
- Claude 本機的記憶檔繼續使用；Notion 是跨裝置、可供創辦人閱讀的版本。

---

*此文件由 Yuna（COO）核准，Brian（CEO）確認生效。2026-09-26 起，任何更新由創辦人指示即生效，毋須簽署（§18）。*
*2026-09-04 修訂（§3／§5／§8／§16.E，新增 §17）：Brian ＋ Yuna 雙簽，*
*修訂案全文及裁決紀錄見 `docs/charter-amendment-2026-09-04.md`。*
*2026-09-05 修訂（§3／§5／§8／§8.2／§11／§17）：Brian 裁決撤銷整套收費框架，*
*代碼、頁面、五張資料表、`stripe` 套件同日剷除。*
*✅ Yuna 於 2026-09-09 副署 —— 雙簽完成，本次修訂正式生效。*
*（原標記「⬜ 待 Yuna 副署」由 2026-09-05 掛到 2026-09-09，共四日。*
*四日之間條文係生效定未生效，冇人答得到 —— 呢個就係「裁決同副署之間嗰段空窗」，*
*同 §16.E「批准同落實係兩件事」係同一個病。往後撤銷類修訂應同日雙簽。）*
*2026-09-08 修訂（§16.E 約束 5 ＋ 執行第 1 點）：Yuna 裁決，答案原文准許上雲，*
*僅限 `dse_active_session` ＋ `dse_reverse_log` 兩個 key，用於跨裝置連續性。*
*代碼、測試、私隱頁同日一齊改。全文見 `docs/charter-amendment-2026-09-08.md`。*
*✅ Brian 於 2026-09-09 副署 —— 雙簽完成，本次修訂正式生效。*
*2026-09-09 修訂（§7）：一節練習題數由 20 減到 10，Yuna 裁決。*
*理由：74% 嘅節冇做夠 20 題，中位數停喺 15；另為 ADHD 學生對齊番茄鐘。*
*目標將中途離開率由 74% 壓到 30%，2026-10-09 覆檢。*
*2026-09-09 修訂（§7 ＋ 新增 §7.2）：Yuna 裁決剷除 30 秒反思鎖，為期兩個月，*
*2026-11-09 憑逐週正確率 curve 覆檢。三維自診維持生效。原條文存檔保留。*
*按 §18（2026-09-26），本修訂毋須副署，自 2026-09-09 裁決起生效。*
*2026-09-26 修訂（新增 §18、§19）：Yuna 指示，即時生效。*
*2026-09-26 修訂（§7 刪「零故障保證」、§14 改為莫蘭迪）：Yuna 2026-09-21 表態，按 §18 生效。§3 Next.js 版本號 Yuna 不同意改，維持原文。*
*DSE LEVEL UP — 戰場只有一個，目標只有一個：幫香港考生贏最後三十分。*
