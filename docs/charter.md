...
### 16.E 個人逐課題正確率 —— L2 平台可讀層，禁止上雲

> **任何含「`user_id` ＋ 課題 ＋ 正確率／錯誤次數」組合嘅 schema 提案，一律駁回，
> 唔需要再評估。**

**裁決日期：** 2026-08-25。**實作證據：** commit `9adc536`（2026-08-24）
將 `dse_topic_stats`（個人逐課題正確率）同 `dse_active_session`（答案原文）
由上雲白名單**剔走**。現時 `lib/sync.ts` 只上傳兩個 key：
`dse_progress` 同 `dse_free_attempts_total`。

#### 點解要寫入憲章而唔係靠記得

呢個提案到 2026-08-27 已經係**第七次**出現，載體包括：規格書 v2.0／v2.1、
