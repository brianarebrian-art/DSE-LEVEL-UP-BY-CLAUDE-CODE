  ...
  return (
    <>
      ...
      <div className="sr-only">
        ...
        <p>共 {total} 條獨立改寫題目（多項選擇 {mc} 條、書寫題 {written} 條），涵蓋 {subjects.length} 個 DSE 科目。答錯任何一題都可以從概念盲區、審題陷阱、運算粗心三方面自我診斷。書寫題永不機器批改，交卷後對照參考答案自評。所有題目為原創改寫，並非香港考試及評核局（HKEAA）官方試題。{/* i18n-exempt: 雙語已並列，緊接下段英文（server component 冇 locale） */}</p>
        <p>
          {total} independently rewritten questions across {subjects.length} HKDSE subjects
          ({mc} multiple-choice, {written} written-response). A three-way self-diagnosis follows any wrong
