    ...
    <div className="min-h-screen px-4 py-12 bg-surface text-ink-soft">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-surface-raised border border-line rounded-2xl p-6 mb-6">
          <h2 className="font-medium mb-2 text-ink">{tr('題目', 'The Task')}</h2>
        ...
        <div className="bg-surface-raised border border-line rounded-2xl p-6 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-medium text-ink">{tr('5 個高分詞組 Vocabulary Booster', '5 Vocabulary Boosters')}</h2>
          </div>
          <div className="space-y-3">
            {VOCAB.map((v) => (
              <button
                ...
                className="w-full text-left group bg-surface-sunken hover:bg-surface-sunken border border-line hover:border-violet/40 rounded-xl px-4 py-3 transition-all"
              >
        ...
        <div className="bg-surface-raised border border-line rounded-2xl p-6">
            ...
            {DOMAINS.map((d) => (
              <div key={d.key}>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-medium text-ink text-sm">{tr(d.zh, d.en)}</span>
                  {scores[d.key] > 0 && (
                    <span className="text-xs text-violet font-medium">{scores[d.key]} / 7</span>
                  )}
                </div>
          ...
          {/* Overall band */}
          {avg > 0 && (
            <div className="mt-6 p-4 bg-violet/[0.06] border border-violet/25 rounded-xl text-center">
              <div className="text-xs text-ink-muted mb-1">{tr('綜合自評', 'Overall self-assessment')}</div>
              <div className="text-2xl font-medium text-violet">{avg.toFixed(1)} / 7</div>
              <div className="text-sm text-violet mt-1">{bandLabel(avg, en)}</div>
            </div>
          )}
          {rated > 0 && rated < 3 && (
