          ...
          Array.from({ length: t.target }).map((_, i) => {
            ...
            return (
              <span
                ...
                style={{ background: filled ? t.color : '#1e293b', animationDuration: '300ms', animationIterationCount: filled ? 2 : undefined }}
              />
            )
          }),
        )}
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3 text-xs text-slate-400">
        {TIERS.map((t) => (
          <span key={t.key} className="inline-flex items-center gap-1.5">
            ...
            {s[t.key] > t.target && <span className="text-slate-500">(+{s[t.key] - t.target})</span>}
          </span>
        ))}
      </div>
    </div>
