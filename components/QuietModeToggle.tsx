...
// Dashboard 嘅安靜模式開關。見 lib/quietMode.ts 檔頭：收埋段位／EXP 同溫習時數。
...
export default function QuietModeToggle() {
  ...
  return (
    ...
    >
      ...
      {quiet
        ? en
          ? 'Quiet mode on — rank, EXP and study time hidden'
          : '安靜模式：已收埋段位、EXP 同溫習時數'
        : en
          ? 'Quiet mode — hide rank, EXP and study time'
          : '安靜模式：收埋段位、EXP 同溫習時數'}
    </button>
  )
}
