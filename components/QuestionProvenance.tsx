...
// 2026-09-25: the named review records were deleted on Yuna's instruction, so the
// "reviewed one-by-one by <name>" state no longer exists. Two states remain:
// automated checks, and pending (a specific problem found but not yet fixed).
      ...
      <p className="text-[11px] text-ink-muted leading-relaxed">
        ...
        <span
          ...
          className={`inline-block w-1.5 h-1.5 rounded-full mr-1.5 align-middle ${
            pending.length ? 'bg-gold' : 'bg-ink-soft'
          }`}
        />
        {pending.length ? (
          <>
          ...
          <>
            ...
            {' — '}
            {en
              ? 'terminology and register, structure and difficulty mix, format and completeness.'
              : '術語與書面語、結構與難度比例、格式與材料齊備。'}
          </>
        )}
        {' '}
