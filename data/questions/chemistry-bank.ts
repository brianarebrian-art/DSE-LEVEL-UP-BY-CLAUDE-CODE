...
COMP.forEach((c, ci) => {
  for (let nn = 1; nn <= 2; nn++) {
    ...
    add(`cb_e1_${ci}_${nn}`, T.mole, FW.quantity, 'easy',
      ...
      [`$n = \\dfrac{m}{M} = \\dfrac{${m}}{${c.mr}} = ${nn}$ mol。陷阱：$${m * c.mr}$ 用咗乘法；$${round(c.mr / m, 3)}$ 上下倒轉。`,
...
COMP.forEach((c, ci) => {
  for (const nn of [3]) {
    ...
    add(`cb_e2_${ci}_${nn}`, T.mole, FW.quantity, 'easy',
      ...
      [`$m = nM = ${nn} \\times ${c.mr} = ${m}$ g。陷阱：$${round(nn / c.mr, 3)}$ 用咗除法；$${c.mr}$ 漏咗 $\\times n$。`,
...
;([
  ...
  add(`cb_e3_${i}`, T.formula, FW.calc, 'easy',
    ...
    [`質量百分比 $= \\dfrac{\\text{該元素質量}}{M_r} \\times 100\\% = \\dfrac{${c.part}}{${c.mr}} \\times 100\\% = ${pct}\\%$。陷阱：$${round(((c.mr - c.part) / c.mr) * 100, 1)}\\%$ 係其餘部分；$${round(c.part / c.mr, 3)}$ 漏咗 $\\times 100\\%$。`,
...
for (let nn = 1; nn <= 6; nn++) {
  for (const V of [2, 4, 5]) {
    ...
    add(`cb_m1_${nn}_${V}`, T.concentration, FW.calc, 'medium',
      ...
      [`$c = \\dfrac{n}{V} = \\dfrac{${nn}}{${V}} = ${round(conc, 3)}$ mol/dm³。陷阱：$${round(V / nn, 3)}$ 上下倒轉；$${nn * V}$ 用咗乘法。`,
  ...
  for (const nn of [1, 2]) {
    for (const V of [2, 5]) {
      ...
      add(`cb_m2_${ci}_${nn}_${V}`, T.concentration, FW.calc, 'medium',
        ...
        [`先求摩爾：$n = \\dfrac{${m}}{${c.mr}} = ${nn}$ mol，再 $c = \\dfrac{n}{V} = \\dfrac{${nn}}{${V}} = ${round(conc, 3)}$ mol/dm³。陷阱：$${round(m / V, 3)}$ 用咗質量而唔係摩爾；$${nn}$ 漏咗 $\\div V$。`,
...
for (const nn of [2, 3, 4, 5, 0.5]) {
  ...
  add(`cb_m3_${String(nn).replace('.', '_')}`, T.mole, FW.quantity, 'medium',
    ...
    [`粒子數 $= n \\times N_A = ${nn} \\times 6.02\\times10^{23} = ${val}\\times10^{23}$。陷阱：$6.02\\times10^{23}$ 漏咗 $\\times n$；$${val}\\times10^{-23}$ 指數符號錯。`,
  ...
  .forEach((c, ci) => {
    for (const nn of [0.5, 1, 1.5, 2, 2.5, 3]) {
      ...
      add(`cb_h1_${ci}_${String(nn).replace('.', '_')}`, T.gas, FW.reaction, 'hard',
        ...
        [`先求摩爾 $n = \\dfrac{${m}}{${c.mr}} = ${nn}$ mol，再 $V = n \\times 24 = ${nn} \\times 24 = ${round(V, 2)}$ dm³。陷阱：$${nn}$ 漏咗 $\\times 24$；$${round(24 * m, 2)}$ 用咗質量而唔係摩爾。`,
  ...
  for (const cb of [2, 3, 4]) {
    ...
    for (const Va of [20, 24, 30, 36, 40, 48, 60]) {
      ...
      add(`cb_h2_${ca}_${cb}_${Va}`, T.stoichiometry, FW.reaction, 'hard',
        ...
        [`酸鹼摩爾比 $1:1$：$c_aV_a = c_bV_b$ ⇒ $V_b = \\dfrac{c_aV_a}{c_b} = \\dfrac{${ca}\\times${Va}}{${cb}} = ${Vb}$ cm³。陷阱：$${round((cb * Va) / ca, 2)}$ 將兩個濃度倒轉；$${ca * Va}$ 漏咗 $\\div c_b$；$${Va}$ 當兩者體積相等。`,
...
for (const m of [25, 50, 75, 100, 125, 150, 175, 200, 250, 300]) {
  ...
  add(`cb_h3_${m}`, T.stoichiometry, FW.reaction, 'hard',
    ...
    [`摩爾比 $1:1$。$n(\\text{CaCO}_3) = \\dfrac{${m}}{100}$，$m(\\text{CaO}) = n \\times 56 = \\dfrac{${m}\\times56}{100} = ${cao}$ g。陷阱：$${co2}$ 係放出的 $\\text{CO}_2$ 質量（$56+44=100$）；$${m}$ 當冇質量損失。`,
