...
limits.push(
  q(id('lc'), T.limits, FW.transform, 'easy', 2023, 2,
    ...
    [optm('1'), optm('0'), optm('\\infty'), opt('不存在', 'Does not exist')],
  ...
  q(id('lc'), T.limits, FW.transform, 'medium', 2021, 3,
    ...
    [bi('左右極限相等且等於函數值 / left and right limits are equal and equal the function value'),
      bi('函數無定義 / the function is undefined there'),
      bi('導數不存在 / the derivative does not exist'),
      bi('極限為無限大 / the limit is infinite')],
...
matrices.push(
  ...
  q(id('mc'), T.matrices, FW.decompose, 'medium', 2022, 3,
    ...
    [bi('奇異（不可逆）/ singular (non-invertible)'),
      bi('可逆 / invertible'),
      bi('單位矩陣 / the identity'),
      bi('對稱 / symmetric')],
  ...
  q(id('mc'), T.matrices, FW.decompose, 'hard', 2021, 4,
    ...
    [bi('行列式不為零 / has a non-zero determinant'),
      bi('行列式為零 / has a zero determinant'),
      bi('是零矩陣 / is the zero matrix'),
      bi('沒有逆矩陣 / has no inverse')],
...
vectors.push(
  ...
  q(id('vc'), T.vectors, FW.geometry, 'medium', 2022, 3,
    ...
    [bi('純量 / scalar'), bi('向量 / vector'),
      bi('矩陣 / matrix'), bi('零 / zero only')],
  ...
  q(id('vc'), T.vectors, FW.geometry, 'easy', 2021, 2,
    ...
    [optm('0'), optm('1'), optm('\\infty'), opt('未定義', 'Undefined')],
...
const induction: Question[] = [
  q(id('mi'), T.induction, FW.decompose, 'medium', 2023, 3,
    ...
    [bi('驗證 $n=1$（或起始值）成立 / verify the statement for $n=1$ (or the starting value)'),
      bi('假設 $n=k$ 成立 / assume it holds for $n=k$'),
      bi('證明 $n=k+1$ / prove it for $n=k+1$'),
      bi('對所有 $n$ 求和 / sum over all $n$')],
  ...
  q(id('mi'), T.induction, FW.decompose, 'medium', 2022, 3,
    ...
    [bi('假設 $n=k$ 成立，證明 $n=k+1$ 成立 / assume $n=k$ holds and prove $n=k+1$'),
      bi('只驗證 $n=1$ / only check $n=1$'),
      bi('代入所有數值 / substitute all values'),
      bi('求導數 / differentiate')],
  ...
  q(id('mi'), T.induction, FW.decompose, 'medium', 2020, 3,
    ...
    [bi('所有正整數 $n$ / all positive integers $n$'),
      bi('所有實數 / all real numbers'),
      bi('複數 / complex numbers'),
      bi('無理數 / irrational numbers')],
  ...
  q(id('mi'), T.induction, FW.decompose, 'medium', 2022, 3,
    ...
    [bi('證明不完整，結論不成立 / the proof is incomplete and invalid'),
      bi('證明仍然完整 / the proof is still complete'),
      bi('不需基礎步 / the base case is unnecessary'),
      bi('自動對所有 $n$ 成立 / it holds for all $n$ automatically')],
  ...
  q(id('mi'), T.induction, FW.decompose, 'hard', 2021, 4,
    ...
    [bi('命題對 $n=k$ 成立 / the statement holds for $n=k$'),
      bi('命題對所有 $n$ 成立 / the statement holds for all $n$'),
      bi('命題對 $n=k+1$ 成立 / the statement holds for $n=k+1$'),
      bi('命題不成立 / the statement is false')],
  ...
  q(id('mi'), T.induction, FW.decompose, 'medium', 2020, 3,
    ...
    [bi('一列骨牌相繼倒下 / a line of dominoes toppling'),
      bi('擲骰子 / rolling dice'),
      bi('一次過驗證 / checking all at once'),
      bi('猜測答案 / guessing the answer')],
...
const binomial: Question[] = [
  ...
  q(id('bin'), T.binomial, FW.decompose, 'easy', 2023, 2,
    ...
    [optmPair('1', '1'), optmPair('0', '1'), optmPair('n', '1'), optmPair('1', 'n')],
