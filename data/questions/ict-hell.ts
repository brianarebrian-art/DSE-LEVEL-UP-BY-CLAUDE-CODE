...
const logic: Question[] = [
  ...
  q(id('lo'), T.logic, FW.apply, 'hard', 2023, 2,
    ...
    [opt('10', '10'),
      ...
      opt('500', '500'),
      opt('100', '100')],
    C('二分搜尋每次比較把搜尋範圍減半，最壞次數 ≈ ⌈log₂ 1000⌉。2¹⁰ = 1024 > 1000 > 512 = 2⁹，故約 10 次。\n\n【陷阱】1000 係「線性搜尋」最壞次數；500 係線性搜尋平均；100 無對應算法依據。二分搜尋的威力正在於對數級增長。',
...
const net: Question[] = [
  q(id('ne'), T.net, FW.apply, 'hard', 2024, 2,
    ...
    [opt('12 秒', '12 seconds'),
      ...
      opt('96 秒', '96 seconds'),
      opt('8 秒', '8 seconds')],
    C('關鍵是位元組 vs 位元：12 MB = 12 × 8 = 96 兆位元 (Mb)。時間 = 96 Mb ÷ 8 Mbps = 12 秒。\n\n【陷阱】1.5 秒 = 12 ÷ 8，直接用 MB 除 Mbps，漏咗 ×8 換算（最常見錯誤）；96 秒誤把分子分母關係搞錯；8 秒無依據。',
