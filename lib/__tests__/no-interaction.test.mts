...
test('/focus 嘅房號維持純前端 —— 呢條係界線，唔係例外', () => {
  // 自律房間畀學生 share 一個房號一齊溫，但各自計時、零資料交換。
  // 佢喺線內，因為冇任何內容由一個用戶去到另一個用戶。呢條測試釘死呢點：
  // 一旦有人加咗 fetch 去同步房間狀態，佢就變成互動功能。
  const src = stripComments(fs.readFileSync('app/focus/page.tsx', 'utf8'))
  assert.doesNotMatch(src, /fetch\(\s*['"`]\/api\//, '/focus 開始向自家 API 攞／send 房間狀態 —— 咁就唔再係純前端')
})
