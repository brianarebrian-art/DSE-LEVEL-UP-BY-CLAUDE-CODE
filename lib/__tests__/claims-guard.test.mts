...
const MUST_CATCH: [label: string, sentence: string][] = [
  ['引用具體官方試題題號', '<p>2023 DSE Math Paper 1 Q1</p>'],
  ['話自己嘅題係真題', '<p>一條 2023 DSE 真題，拆解核心邏輯</p>'],
  // ⚠️ 樣本要跟【真實違規寫法】：呢兩個係標籤，喺原始碼入面係一個字串值，
  // 唔係 JSX 文字。個閘特登只攔「成個字串值就係佢」——
  // 見 claims-guard.mjs 嗰條規則點解要咁窄。
  ['英文標籤聲稱官方', "officialLabel: 'Official question',"],
  ['中文標籤聲稱官方', "officialLabel: '官方試題',"],
...
const MUST_PASS: [label: string, sentence: string][] = [
  // 描述改寫方法（睇官方卷抽考點）唔係聲稱自己展示緊官方原文。
  // 第一版 past-paper-claim 規則喺呢句誤報過，所以鎖低。
  ['描述改寫方法', "'分析每道官方試題的考核能力點（唔係答案，係「考緊乜嘢」）',"],
  ['英文版描述方法', "'Analyse each official question rsquo s assessed ability',"],
  // 呢兩句係【指向 HKEAA】嘅正確免責，唔可以被 past-paper-claim 誤攔。
  ['免責：官方試題以考評局為準', '<p>官方試題同成績以佢哋為準。</p>'],
  ['免責：叫人去 HKEAA 攞', '<p>官方歷屆試題請前往 HKEAA 網站下載。</p>'],
