...
三部分：① 累計時間／平均每題／有練習日數（30 日窗口）；② 最近 14 日逐日條形圖；③ 四時段正確率對照（深夜／早上／下午／夜晚）。

**大愛紅線落實：**
- 冇「建議時數」、冇進度條催促、冇同其他考生比較。整張卡係【描述】唔係【評分】。
- **休息日照樣佔一格**（極淡底色），唔會因為冇練習而喺圖上消失 —— 文案寫明「平嗰啲日子係休息日 —— 佢哋係節奏嘅一部分，唔係缺口」。
- **樣本唔夠就唔出結論**：一個時段要 ≥20 題、而且要至少兩個時段有數據先會講「你最清醒嘅時段」。用兩三題去講一個學生嘅生理節奏＝虛構統計，同憲章禁假數據係同一條線。唔夠就老實講「而家仲講唔準」。
- 零樣本時段顯示「暫 0 題」而唔係「0%」（`accuracy` 用 `null` 唔用 `0`），避免將「未做過」睇成「全錯」。

**測試捉到一個真 bug**：`formatDuration` 原本用 `Math.max(0, Math.round(seconds))` 夾底，但 `Math.max(0, NaN)` 係 `NaN`（唔會被夾住），跟住每個比較都 false，最後會渲染出「NaN 分鐘」落畫面。已改為先 `Number.isFinite` 再夾。

20 條測試（`npm test` 155 → **175**）。qa 三閘綠、tsc 0、build 綠 81/81。瀏覽器實測（種入 15 份卷後已清走）：累計 3 小時 25 分、平均每題 41 秒、10/30 日、14 格條形圖 10 格有數據、正確辨認「夜晚 90%／200 題」為最佳時段而下午 55% 唔會被誤選、深夜同早上顯示「暫 0 題」；切英文全出英文零中文洩漏。
...
- **架構**：本體係 `public/supermarket/`（單一 HTML + 8 個 CSS + 11 個 JS，**零 npm 依賴、零外部 CDN、零圖片**，全部 CSS/SVG 幾何繪製 + Web Audio 原生合成），完全依 spec §5.1／§5.5。`/relax/virtual-supermarket` 改為薄 React 殼，用**同源 iframe** 載入 —— 咁 `app/relax/layout.tsx` 嘅緊急熱線橫幅同「一撳離開」照樣包住個超市（兩樣係 NON-NEGOTIABLE），靜態頁自己亦另帶一份熱線以防直連。
- **🔒 安全 header 變更（必須留意）**：全站原本 `/:path*` 落 `X-Frame-Options: DENY` + `frame-ancestors 'none'`，會連同源 iframe 都封死。`next.config.ts` 新增一條**只針對 `/supermarket/:path*`** 嘅規則，放寬至 `SAMEORIGIN` / `frame-ancestors 'self'` —— 只准本網域嵌自己，外站一樣嵌唔到；其餘 header（nosniff／HSTS／Referrer-Policy）不變。實測 `/relax` 仍然回 `DENY`。
- **溫習幣（§4.1）**：完全閉環，零充值、零兌換、零轉讓。每日首次 +50、連續三日 +200、完成一節溫習 +100、首次擴建 +500、CHA 評分 +100（升級再 +300）。「完成一節溫習」讀主平台 `dse_progress`（`AttemptRecord[]`，有 timestamp）—— 同源共用 localStorage，**毋須 postMessage**（spec §5.2 禁 postMessage，此處遵守）；只補發「上次派幣之後」嘅新紀錄，唔重複派亦唔回溯。`dse_result` 因為冇 timestamp 而未採用。
- **連續三日獎勵嘅處理**：只喺啱啱儲夠三日嗰日派一次，**畫面永遠唔顯示斷咗幾多日** —— 避免重蹈 2026-07-31「連續打卡歸零」嗰個壓力源。
- **已落地**：淘寶式選貨（272 件商品／19 分類／搜尋）· 商品詳情 · 購物車增減 · 支付確認 · 30 秒速遞動畫（純 CSS 城市剪影＋道路＋擺腿速遞員＋進度條＋文字輪播＋跳過掣）· 儲物櫃 · 等角房間家具擺放／轉向／收返 · 五級擴建 · 風水三區 · 溫馨家協會每週評分 · DIY 合成 · SEN 專注模式 · 高對比模式 · 音效開關 · 備份碼導出導入 · 3 分鐘無操作溫柔提示。
- **未做（spec v1.1，明確留白）**：社交分享碼「參觀他人小窩」—— 8 位碼需要伺服器解析，但 §5.1 禁後端，規格內互相矛盾，故整項未實作；音響播放面板亦未做。
- **刻意偏離 spec 一處**：§3.3 寫 `--font-body: 9px`（≈6.75pt）。9px 正文同 §3.2「讀寫障礙友善」及 §5.4「WCAG AA」直接矛盾，改用 15px 基準（SEN 模式 18px），已喺 `css/base.css` 檔頭註明理由同還原方法。
- **商品檔清洗**（`data/relax/supermarketProducts.ts` → 產生 `js/products.js`）：剷走全部 242 件嘅 `rating`／`reviewCount`（虛構評價，違憲章 §8）；**61 個真實商標改通用名**（維他／嘉頓／雀巢／李錦記／出前一丁／合味道／可口可樂／瑞士蓮／金莎／M&M's／立頓／川寧／桂格／獅球唛／家樂牌／多芬／李施德林／曼秀雷敦／唯潔雅／威露士／斧頭牌／地捫／泰森／日清／Barilla／Chobani／安佳／亨氏／UNO 等），檔頭「來源：惠康 + 百佳」刪走，描述欄「MSC認證」改「可持續捕撈」；順帶修好原檔一個未跳脫撇號嘅語法錯（該檔從未被 import 故一直冇 build 到）。價格由港元經 `8×√HKD` 映射為溫習幣（18–500），介面另有 30 件家具（100–500 幣）。
- **修正一個實測揪到嘅 bug**：`home.js` 觸發風水時 `S.modal()` 之後直接 `return`，冇重繪 —— 家具同發光要離開再入返先見到。改為先 `S.render()` 再彈提示。
- **驗收（實跑，非預期值）**：`npm test` **67/67**（v1 嘅 13 條心水清單測試連同組件一併移除，功能已由儲物櫃取代）。`npm run qa` 三閘全綠。`npm run build --webpack` 綠，**81/81 SSG**。靜態資源 **116 KB**（spec §5.3 上限 500KB）。瀏覽器實測：買嘢扣幣正確（3000−260=2740）→ 速遞畫面有速遞員／進度條／跳過掣 → 送達入儲物櫃 → 擺入 6×6 房間 → 風水「✨ 小確幸」觸發並即時發光 12 格 → CHA 評分顯示「萌芽」正向評語且**唔顯示具體分數** → SEN 模式 body 18px。console 零 error；375px 手機零橫向捲、**0 個掣細過 44px**、275 個 `aria-label`；全頁零 HKD／美元符號、零排行榜段位 PVP、零 FAIL／❌；localStorage 只用 `dse-supermarket-v2` 一個鍵。
...
## 2026-07-31c — 晨曦憲章 R3（不予取代，改為併入；「2027 後收費」不採納）

> ⚠️ 憲章本體喺 `.claude/skills/dse-level-up-claude-code-ultimate-prompt/SKILL.md`，
> **喺 repo 之外，唔受 git 追蹤**。此節為版本控制內嘅唯一書面記錄。

- **背景**：外部兩份「Perfect Prompt」（Rev 1／Rev 2）擬**取代**現有憲章 skill。逐條核實後**不予取代**。
- **不取代嘅理由（三項倒退）**：兩份都比現行 skill 薄（379 行 vs 517 行），且會令已完成嘅更正倒退 ——
  ① 長答自動批改復發（現行 §5 已於 2026-07-19 降級為自我對照）；
  ② 現行 §3.3 明文「Body 最小 16px，絕對禁止 9pt/7pt（撞 SEN 可讀性紅線）」，Rev 2 改成 14px + Caption 7pt；
  ③ 現行 §3 係完整 light-first 設計系統，Rev 2 §4 改回「暗底霓虹」為設計哲學但色表又係 light-first，自相矛盾。
- **改為併入嘅有效修正**（憲章升版 `2026-07-18-ULTIMATE` → `2026-07-31-R3`）：
  - **§3.2 色表刪 hex，改指向 `app/globals.css` 的 `@theme`**。此前同時存在三套值：本檔硬寫嘅一套、`bb27544` 提高後嘅一套、外部規格另寫嘅第三套。只保留唔隨值變嘅四條鐵律（`ink-faint` 豁免範圍、霓虹色 cyber-only 且附實測 1.34／1.23／3.67／3.95、取三底面最差值、改完跑 `contrast-audit.js` 驗渲染色）。憲章所列 13 個 token 已逐個核實存在。
  - **§3.2／§13／附錄三處硬寫 accent hex 一併清走**（`#008B84`、`#00A8A0` 漸變與檢查項），改用 `var(--color-accent)`。
  - **§5 中文科補概念網實況**：12 篇 · **58 節點**（非 29）· **4 類且列明真實 `node_types`**（`theme` / `technique` / `line` / `figure`）—— 外部文件三次寫成「29 節點／5 類」並虛構類名。另註明節點數 × 類別數 **不是**可用組合數。
  - **§5／§6 刪「只支援 `type:"mc"`」**，補 `text`／`long` 規格：無 `options`／無 `correctIndex`、須有 `referenceAnswer`、學生自評、自評結果不得混入 accuracy／等級預測。
  - **§9 Bella KPI「長答題關鍵字覆蓋率 90%+」刪除**（撞 §2）；**§9 Arthur「嚴打盲抄」→「溫和非評分提示」**（同 §5 對齊）。
  - **§14 指令表逐個標實際安裝狀態**（8 個 ✅／6 個 ❌ 未安裝）；**`/SIMPLIFY` 永久刪除** —— 呢個環境有同名內建 skill，功能係 code review **＋自動改檔案**，同「解析簡化」無關。
  - **§16 哲學表「減壓緩衝區」→「呼吸空間（`/relax`，產品名）」**，對齊 2026-07-15 CEO 命名指令。
- **「2027 年 DSE 完結後正式收費」不採納**（Yuna 2026-07-31 拍板）。該句係 Rev 2 新增，唔喺 Rev 1 亦唔喺 `CLAUDE.md`；一旦寫入系統前置提示，之後每個 session 都會當佢係既定政策。§7 維持「100% 免費、無廣告、無訂閱、無付費牆、無 Premium Plan」。
- 完整審核見 `docs/REVIEW-perfect-prompt-2026-07-31.md`。
